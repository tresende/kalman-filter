...
function Map() {
  ...
  const filterData = (inputData: number[][], threshold: number) => {
  ...
  const filterDataInputText = (text: string) => {
    const inputData = text
      ...
      .split('\n')
      .map((line) => {
        const [lat, long, time] = line.split(',').map(Number)
    ...
    filterData(inputData, distanceThreshold)
  }

  useEffect(() => {
    filterData(data, distanceThreshold)
  }, [distanceThreshold])
    ...
    <div className="App">
      <div className="flex">
        <div className="flex flex-col p-2">
          ...
          <div className="flex justify-center">
            <textarea
              ...
              rows={10}
              placeholder="Insira os dados de latitude, longitude e timestamp aqui..."
          ...
          <div className="my-4 text-center">
            ...
            <input type="range" min="0" max="300" value={distanceThreshold} onChange={handleRangeChange} />
        ...
        <GoogleMap mapContainerStyle={{ width: '100%', height: '100vh', overflow: 'none' }} center={center} zoom={15}>
          {(showFiltered ? filteredData : data).map(([lat, long, time], index) => (
            <Marker
              key={index}
